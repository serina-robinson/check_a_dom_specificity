...
# dendcut10_df <- data.frame(cbind(names(dendcut10), dendcut10))
# colnames(dendcut10_df) <- c("labnames", "clusters")
# write_csv(dendcut10_df, "output/dendcut10_df.csv")


# dend2 %>% ladderize %>%  plot(horiz = TRUE); abline(v = .2, col = 2, lty = 2)
