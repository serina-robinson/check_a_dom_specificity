# Install packages
pacman::p_load("Biostrings", "DECIPHER", "ggtree", "randomcoloR", "stringr", "tidyverse", "RColorBrewer", "readxl", "gplots")
...
dd_fin <- data.frame(cbind(p2$data$label, p2$data$class, p2$data$isTip)) %>%
  dplyr::filter(X3 == T) %>%
  dplyr::mutate(color = pal3[as.factor(X2)]) %>%
  dplyr::select(-X3)

head(dd_fin$color)
colnames(dd_fin) <- c("label", "taxonomic_class", "color")

write_tsv(dd_fin, "data/7674_sp2_and_antismashdb_v2_60seqs_taxonomic_annotation_file.txt", col_names = T)
table(dd_fin$color)
