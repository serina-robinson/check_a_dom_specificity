...
dd_annot <- dd %>% 
  add_count(grp) %>%
  dplyr::mutate(broad_grp = case_when((n < 8) ~ "rare_aa",
                TRUE ~ grp)) %>%
...
# n <- length(unique(p2$data$broad_grp))
# set.seed(1234)
# palette <- distinctColorPalette(n)
# pal4 <- distinctColorPalette(n)
# pal4[pal4 == "#E79865"] <- "gray68"
# pal4[pal4 == "#D6D3E2"] <- "#E79865"

tail(p2$data$tipsize)

p2$data$label
# Make a tree by coloring the branches of interest
p2 <- pl %<+% dd_annot
p2$data$tipsize[p2$data$isTip]
table(p2$data$tipsize[p2$data$isTip])
...
pdf("data/7610_FastTree_sp2_antismashdb_v2_seqs_unlabeled_circular_nodes_labeled.pdf", width = 10, height = 10)
par(mar=c(0.001,0.001,0.001,0.001))
ptree
dev.off()

p2$data$broad_grp != "UNKNOWN"
