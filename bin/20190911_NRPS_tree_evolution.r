# Install packages
pacman::p_load("Biostrings", "DECIPHER", "ggtree", "randomcoloR", "stringr", "plyr",
...
dend_enframe <- enframe(dendcut_ll)
head(dend_enframe)


dend_enframe$value[4]
dend_df2 <- ldply(dendcut_ll, data.frame)

dend_cbind <- cbind(names(unlist(dendcut)), dend_df2)
colnames(dend_cbind)
dend_df <- do.call(rbind.data.frame, dendcut_ll)
